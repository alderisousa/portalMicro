...
export function MarketStockDashboard({ accountId, onBack }: Props) {
  ...
  const changeStore = async (nextStoreId: string) => {
    ...
    catch (cause) { console.error('Falha ao consultar estoque:', cause); setError('Não foi possível consultar esta loja.') }
    finally { setLoading(false) }
  }
  ...
  const startDraft = async () => {
    ...
    try {
      ...
      setError(isVersionConflict(cause) ? 'Já existe um inventário em andamento nesta loja. Recarregue para continuar.' : 'Não foi possível iniciar o inventário.')
  ...
  return <div className="market-stock-dashboard">
    ...
    <div className="market-dashboard-filter"><label htmlFor="market-stock-store">Loja</label><select id="market-stock-store" value={storeId} onChange={(event) => void changeStore(event.target.value)} disabled={loading || counting}><option value="">Selecione uma loja</option>{context.access.stores.map((store) => <option key={store.id} value={store.id}>{store.external_code ? `${store.external_code} — ` : ''}{store.name}</option>)}</select>{selectedStore && <span>{selectedStore.name}</span>}</div>
