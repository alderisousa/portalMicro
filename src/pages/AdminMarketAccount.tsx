...
import type { MarketAccount, MarketAccountMember, MarketAccountStatus, MarketMemberRole, MarketMemberStatus, MarketPlanCode, MarketStore, MarketStoreInput, MarketStoreStatus } from '../types/market'
...
const emptyStore: MarketStoreInput = { name: '', external_code: null, description: null, status: 'active' }
...
export function AdminMarketAccount({ accountId, onBack }: AdminMarketAccountProps) {
  ...
  const openMemberEdit = (member: MarketAccountMember) => { setEditingMemberId(member.id); setMemberForm({ userId: member.user_id, role: member.role === 'owner' ? 'viewer' : member.role, status: member.status === 'disabled' ? 'disabled' : 'active', allStores: member.all_stores, storeIds: member.store_ids ?? [] }); setShowMemberForm(true); setFeedback(null) }
  ...
  const saveStore = async (event: FormEvent) => {
    ...
    try { if (editingStore === 'new') await createMarketStore(accountId, input); else if (editingStore) await updateMarketStore(accountId, editingStore.id, input); setEditingStore(null); setStores(await listMarketStores(accountId)); setFeedback({ type: 'success', message: editingStore === 'new' ? 'Loja criada com sucesso.' : 'Loja atualizada com sucesso.' }) }
    catch (error) { console.error('Falha ao salvar loja:', error); setFeedback({ type: 'error', message: 'Não foi possível salvar a loja.' }) }
    finally { setSavingAction('') }
  }
  ...
  return <div className="admin-market-account">
    ...
    <section className="admin-market-block"><div className="admin-list-heading"><div><span className="panel-kicker">ACESSOS</span><h2>Usuários da conta</h2></div><button className="button button-small" onClick={openNewMember}><UserPlus size={15} /> Vincular usuário</button></div>
      {showMemberForm && <form className="admin-inline-form" onSubmit={saveMember}><h3>{editingMemberId ? 'Editar acesso' : 'Vincular usuário existente'}</h3>{!editingMemberId && <><label>Buscar usuário<input value={userSearch} onChange={(event) => setUserSearch(event.target.value)} placeholder="Nome ou e-mail" /></label><label className="admin-form-wide">Usuário<select required value={memberForm.userId} onChange={(event) => setMemberForm((current) => ({ ...current, userId: event.target.value }))}><option value="">Selecione</option>{availableUsers.map((user) => <option key={user.user_id} value={user.user_id}>{user.full_name?.trim() || 'Usuário'} — {user.email}</option>)}</select></label></>}<div className="admin-form-row"><label>Perfil<select value={memberForm.role} onChange={(event) => setMemberForm((current) => ({ ...current, role: event.target.value as MemberForm['role'] }))}><option value="admin">Administrador</option><option value="manager">Gerente</option><option value="operator">Operador</option><option value="viewer">Visualizador</option></select></label>{editingMemberId && <label>Status<select value={memberForm.status} onChange={(event) => setMemberForm((current) => ({ ...current, status: event.target.value as MemberForm['status'] }))}><option value="active">Ativo</option><option value="disabled">Desabilitado</option></select></label>}</div><label className="admin-edit-checkbox"><input type="checkbox" checked={memberForm.allStores} onChange={(event) => setMemberForm((current) => ({ ...current, allStores: event.target.checked, storeIds: event.target.checked ? [] : current.storeIds }))} /> Todas as lojas</label>{!memberForm.allStores && <fieldset className="admin-store-access"><legend>Lojas permitidas</legend>{stores.map((store) => <label key={store.id}><input type="checkbox" checked={memberForm.storeIds.includes(store.id)} onChange={() => toggleStore(store.id)} /> {store.name}{store.external_code ? ` — ${store.external_code}` : ''}</label>)}</fieldset>}<div className="admin-form-actions"><button type="button" className="button button-small button-outline" onClick={() => setShowMemberForm(false)}>Cancelar</button><button className="button button-small" disabled={savingAction === 'member'}>{savingAction === 'member' ? 'Salvando...' : editingMemberId ? 'Salvar acesso' : 'Vincular usuário'}</button></div></form>}
      <div className="admin-market-member-list">{members.map((member) => <article key={member.id}><div><strong>{member.full_name?.trim() || member.email || 'Usuário'}</strong><span>{member.email || 'E-mail não informado'}</span></div><dl><div><dt>Perfil</dt><dd>{roleLabels[member.role]}</dd></div><div><dt>Status</dt><dd>{member.status === 'active' ? 'Ativo' : member.status === 'invited' ? 'Convidado' : 'Desabilitado'}</dd></div><div><dt>Acesso</dt><dd>{member.all_stores ? 'Todas as lojas' : (member.store_ids ?? []).map((id) => stores.find((store) => store.id === id)?.name).filter(Boolean).join(', ') || 'Nenhuma loja'}</dd></div></dl>{member.role === 'owner' ? <span className="admin-status published">Proprietário</span> : <button className="button button-small button-outline" onClick={() => openMemberEdit(member)}>Editar acesso</button>}</article>)}</div>
    </section>
    <section className="admin-market-block"><div className="admin-list-heading"><div><span className="panel-kicker">UNIDADES</span><h2>Lojas</h2></div><button className="button button-small" onClick={() => { setEditingStore('new'); setStoreForm(emptyStore) }}><Plus size={15} /> Nova loja</button></div>{editingStore && <form className="admin-inline-form" onSubmit={saveStore}><h3>{editingStore === 'new' ? 'Nova loja' : 'Editar loja'}</h3><div className="admin-form-row"><label>Nome<input required value={storeForm.name} onChange={(event) => setStoreForm((current) => ({ ...current, name: event.target.value }))} /></label><label>Código externo<input value={storeForm.external_code ?? ''} onChange={(event) => setStoreForm((current) => ({ ...current, external_code: event.target.value }))} /></label></div><label className="admin-form-wide">Descrição<textarea value={storeForm.description ?? ''} onChange={(event) => setStoreForm((current) => ({ ...current, description: event.target.value }))} /></label><label>Status<select value={storeForm.status} onChange={(event) => setStoreForm((current) => ({ ...current, status: event.target.value as MarketStoreStatus }))}><option value="active">Ativa</option><option value="inactive">Inativa</option></select></label><div className="admin-form-actions"><button type="button" className="button button-small button-outline" onClick={() => setEditingStore(null)}>Cancelar</button><button className="button button-small" disabled={savingAction === 'store'}>{savingAction === 'store' ? 'Salvando...' : 'Salvar loja'}</button></div></form>}{!stores.length ? <div className="admin-message">Nenhuma loja cadastrada nesta conta.</div> : <div className="admin-store-list">{stores.map((store) => <article key={store.id}><div><strong>{store.name}</strong><span>Código: {store.external_code || 'não informado'}</span>{store.description && <p>{store.description}</p>}</div><span className={`admin-status ${store.status === 'active' ? 'published' : 'paused'}`}>{store.status === 'active' ? 'Ativa' : 'Inativa'}</span><button className="button button-small button-outline" onClick={() => { setEditingStore(store); setStoreForm({ name: store.name, external_code: store.external_code, description: store.description, status: store.status }) }}><Pencil size={15} /> Editar</button></article>)}</div>}</section>
  </div>
}
