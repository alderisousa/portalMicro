...
-- 2026-09-11
--
-- Corrige a semântica de quantidade desconhecida na consolidação.
--
-- Regra:
-- - suggested_quantity NULL significa necessidade ainda não calculável;
-- - se qualquer loja do produto estiver com necessidade desconhecida,
--   o total consolidado também é desconhecido;
-- - necessidade desconhecida nunca deve virar zero;
-- - estoque conhecido do Galpão não resolve uma necessidade desconhecida.

begin;

alter table public.market_replenishment_order_items
    alter column total_suggested_quantity drop not null,
    alter column suggested_purchase_quantity drop not null;

comment on column public.market_replenishment_order_items.total_suggested_quantity is
'Soma das necessidades sugeridas das lojas. NULL quando ao menos uma necessidade participante ainda nao puder ser calculada.';

comment on column public.market_replenishment_order_items.suggested_purchase_quantity is
'Quantidade sugerida para compra apos considerar o Galpao. NULL quando a necessidade consolidada ainda nao puder ser calculada.';
