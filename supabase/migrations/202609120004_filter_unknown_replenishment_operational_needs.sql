-- Reposicao: nao materializa candidatos sem necessidade operacional conhecida.
-- A analise/batch permanece intacta; apenas a lista operacional e filtrada.
begin;
...
begin
    ...
    where c.market_account_id = p_market_account_id
      ...
      and c.suggested_quantity is not null;
    ...
    with product_totals as (
        select
            c.product_id,
            sum(c.suggested_quantity)::numeric(18,4) as total_suggested_quantity
        from public.market_replenishment_candidates c
        where c.market_account_id = p_market_account_id
          and c.run_id = v_run.id
          and c.priority_level in ('critical', 'high', 'medium')
    ...
    split as (
        select
            ...
            )::numeric(18,4) as warehouse_allocated_quantity
    ...
    select
        ...
        (s.suggested_quantity - s.warehouse_allocated_quantity)::numeric(18,4)
    from split s;
