...
-- Migration: piso minimo de volume M7 para SALES_ACCELERATION
-- Data: 2026-09-10
--
-- Contexto:
-- 202609100012_fix_market_replenishment_sales_product_resolution.sql JÁ FOI
-- APLICADA no Supabase de homologação e é imutável a partir de agora.
--
-- Decisão homologada por simulação:
-- - A fórmula de acceleration_pct está correta, mas gera ruído quando a base
--   (M7) é muito pequena (ex.: 1 unidade virando "2 unidades" já é +100%).
-- - Com piso de 3 unidades vendidas em M7, os candidatos de aceleração
--   persistidos caíram de 461 para 101 na amostra real, ficando
--   operacionalmente coerentes.
-- - Default escolhido: 3 unidades em M7.
--
-- market_replenishment_settings (202609100001) já foi aplicada e também é
-- imutável — o novo campo é adicionado via ALTER TABLE nesta migration,
-- preservando o mesmo modelo de configuração já existente: settings globais
-- do Market (store_id NULL) + override por loja (store_id preenchido),
-- resolvido pela mesma precedência já usada no batch (override > padrão).
--
-- SALES_ACCELERATION passa a exigir as 3 condições simultaneamente:
--   1. quantidade bruta vendida em M7 >= acceleration_min_units_m7 (efetivo)
--   2. acceleration_pct >= acceleration_threshold_pct (inalterado)
--   3. avg_daily_m14 >= avg_daily_m30 (inalterado)
-- A condição 1 usa a soma bruta de M7 (sum_m7) já calculada no pipeline por
-- sales_agg — não é derivada de avg_daily_m7 * 7 novamente. Nenhuma outra
-- regra, peso, agravante, classificação, TOP 200, Galpão, suggested_quantity,
-- seleção de fonte, mapping Accesys, segurança, lock ou is_effective foi
-- alterada. Nenhum dado histórico é reprocessado retroativamente por esta
-- migration (candidates de runs anteriores permanecem como estão).

begin;

alter table public.market_replenishment_settings
    add column acceleration_min_units_m7 numeric not null default 3;

alter table public.market_replenishment_settings
    add constraint market_replenishment_settings_acceleration_min_units_m7_ck
    check (acceleration_min_units_m7 >= 0);

comment on column public.market_replenishment_settings.acceleration_min_units_m7 is
'Piso minimo de unidades vendidas em M7 exigido para a regra SALES_ACCELERATION disparar, evitando ruido de percentuais altos sobre base muito pequena. Default 3 (decisao homologada por simulacao: 461 -> 101 candidatos de aceleracao na amostra real).';
