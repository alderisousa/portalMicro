...
-- Migration: corrige o agravante de "aceleração muito forte" para exigir
--            SALES_ACCELERATION efetivamente disparada
-- Data: 2026-09-10
--
-- Contexto:
-- 202609100014_rename_essential_below_minimum_to_below_minimum.sql JÁ FOI
-- APLICADA no Supabase de homologação e é imutável a partir de agora.
--
-- Problema identificado em homologação:
-- Depois do piso mínimo M7 de SALES_ACCELERATION (202609100013), o agravante
-- de "aceleração muito forte" continuava somando +10 olhando só
-- acceleration_pct >= 2 * acceleration_threshold_pct — mesmo quando
-- SALES_ACCELERATION não era uma regra válida para o candidato (ex.: volume
-- M7 abaixo do piso). O agravante nunca deveria valer isoladamente da regra
-- que ele reforça.
--
-- Caso real que revelou o problema:
--   Produto 3 CORACAO CAFE TRADICIONAL 500G, loja EDIFÍCIL MANHATAN:
--   current_stock=5, minimum_stock=6, coverage_days=47.0852,
--   avg_daily_m7=0.142857 (~1 unidade em M7, abaixo de
--   acceleration_min_units_m7=3), acceleration_pct=114.2857.
--   SALES_ACCELERATION corretamente não disparou (volume M7 insuficiente),
--   mas o agravante de aceleração forte ainda somava +10 só pelo percentual,
--   elevando BELOW_MINIMUM de 35 (medium) para 45 (high) indevidamente.
--
-- Correção (só esta condição, nada mais):
--   antes: r.acceleration_pct is not null
--          and r.acceleration_pct >= v_settings.acceleration_threshold_pct * 2
--   depois: r.f_sales_acceleration
--           and r.acceleration_pct is not null
--           and r.acceleration_pct >= v_settings.acceleration_threshold_pct * 2
-- O peso do agravante continua +10. f_sales_acceleration já garante
-- internamente acceleration_pct is not null e >= acceleration_threshold_pct
-- (202609100013) — a condição adicional apenas passa a exigir também que a
-- regra em si tenha disparado (piso M7 + corroboração M14, inalterados).
--
-- Nenhum outro score/regra foi alterado: STOCKOUT, BELOW_MINIMUM,
-- LOW_COVERAGE, SALES_ACCELERATION, ESSENTIAL_NO_RECENT_SALES, os pesos base,
-- os outros 3 agravantes, as faixas critical/high/medium/low, TOP 200,
-- suggested_quantity, Galpão/warehouse_stock, seleção Accesys/legado,
-- mappings, lock, segurança, is_effective e o contrato de falha permanecem
-- byte-idênticos à versão aplicada em 202609100014. Nenhum run histórico é
-- reprocessado por esta migration.
        ...
        loop
            ...
            flagged as (
                select
                    ...
                    (
                        -- BELOW_MINIMUM: nao depende de is_essential (202609100014).
                    ...
                    (
                        -- is_essential permanece exigido aqui e disponivel para usos futuros.
                        s.is_essential
                        and (
            ...
            triggered as (
                select
                    ...
                    (
                        ...
                        -- mais de 2 regras. Aceleracao muito forte (4o agravante,
                        -- linha abaixo) agora exige tambem f_sales_acceleration —
                        -- correcao desta migration: antes disso, um
                        -- acceleration_pct alto isolado (sem volume M7 suficiente
                        -- para a regra disparar) somava +10 indevidamente mesmo
                        -- quando SALES_ACCELERATION nao era candidata valida.
                        ...
                        + (case when r.coverage_days is not null
                                ...
                                then 10 else 0 end)
                        + (case when r.f_sales_acceleration
                                     and r.acceleration_pct is not null
                                     and r.acceleration_pct >= v_settings.acceleration_threshold_pct * 2
                                then 10 else 0 end)
