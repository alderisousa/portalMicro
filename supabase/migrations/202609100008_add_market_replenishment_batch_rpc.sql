...
-- Migration: RPC principal do batch (market_run_replenishment_batch)
-- Data: 2026-09-10
--
-- Objetivo:
-- Analisar as lojas operacionais do Market (store_type = 'store'), calcular as
-- 5 regras funcionais por produto elegível, pontuar/classificar, ordenar por
-- loja e persistir somente o TOP 200 por loja em market_replenishment_candidates,
-- registrando contadores consolidados em market_replenishment_runs e promovendo
-- o run concluído a is_effective da reference_date.
--
-- Fontes de dados reutilizadas (nenhuma tabela paralela criada):
-- - saldo atual: public.market_stock_balance (mesma view usada por
--   market_get_stock_balance). Ausência de linha = sem histórico de estoque
--   (current_stock fica NULL); linha com quantity_on_hand = 0 = ruptura real.
-- - catálogo/loja: public.market_store_products (minimum_stock, is_essential
--   adicionado em 202609100007) + public.market_products (status ativo).
-- - vendas: o Market tem duas fontes comerciais homologadas e mutuamente
--   exclusivas, exatamente como em public.market_get_commercial_dashboard
--   (202609030002_select_market_dashboard_sales_source.sql):
--     * com integração Accesys ativa -> public.market_sales +
--       public.market_sale_items (source_type='api', source_system='accesys');
--     * caso contrário -> public.market_sales_import_rows (join
--       market_sales_imports.status in ('completed','completed_with_pending')),
--       o mesmo filtro usado por market_get_commercial_dashboard_all_locations.
--   Esta migration reproduz a mesma regra de seleção de fonte; não inventa uma
--   terceira fonte nem duplica saldo.
-- - Galpão: public.market_stores.store_type = 'warehouse' (não há tabela
--   separada). warehouse_stock nos candidatos é a soma do saldo (mesma
--   market_stock_balance) de todos os locais warehouse do Market para aquele
--   produto; NULL quando o Market não tem nenhum warehouse.
--
-- Fórmulas (documentadas também no resumo da tarefa):
-- - avg_daily_m7/m14/m30 = soma da quantidade vendida na janela de N dias
--   terminando em p_reference_date, dividida por N (7/14/30). Sempre um
--   número >= 0 (nunca NULL: ausência de vendas na janela = 0).
-- - reference_daily_demand: m7/m14/m30 usam a média correspondente; weighted
--   usa weight_m7*avg_daily_m7 + weight_m14*avg_daily_m14 + weight_m30*avg_daily_m30
--   (pesos efetivos da configuração resolvida).
-- - coverage_days = greatest(current_stock, 0) / reference_daily_demand, somente
--   quando reference_daily_demand > 0 e current_stock é conhecido; caso
--   contrário NULL. O saldo real (inclusive negativo) é preservado sem
--   alteração na própria coluna current_stock; apenas o cálculo de cobertura
--   usa saldo utilizável mínimo 0 (um saldo negativo não pode gerar
--   coverage_days negativo, o que violaria o check da tabela de candidatos).
-- - acceleration_pct = ((avg_daily_m7 - avg_daily_m30) / avg_daily_m30) * 100,
--   somente quando avg_daily_m30 > 0; caso contrário NULL.
-- - suggested_quantity = greatest(0, greatest(coverage_target_days *
--   reference_daily_demand, coalesce(minimum_stock,0)) - greatest(coalesce(current_stock,0), 0)).
--   Mesmo raciocínio de saldo utilizável mínimo 0: ausência de histórico ou
--   saldo negativo nunca aumentam nem diminuem a sugestão além do necessário
--   para atingir a meta a partir de "nada disponível".
--
-- Concorrência: pg_advisory_xact_lock(hashtextextended(...)) por
-- market_account_id + reference_date, no mesmo padrão já usado em
-- market_begin_sales_sync, market_import_purchase_staging etc.
--
-- Janela de vendas: o lookback usado para pré-computar tmp_replenishment_sales
-- é greatest(90, maior essential_no_sale_days efetivo entre as lojas
-- operacionais do Market), calculado antes do loop de lojas. 90 é apenas o
-- piso (cobre M30 com folga); nunca é usado como teto — para qualquer
-- essential_no_sale_days configurado, a regra ESSENTIAL_NO_RECENT_SALES fica
-- exata (a janela consultada é sempre >= ao valor efetivo de cada loja).
--
-- Falha: erros durante o processamento (após o run já existir) são capturados
-- em um bloco exception interno que marca a run como failed (status,
-- finished_at, error_code, error_message) e retorna normalmente — sem
-- re-raise — para não reverter, junto com o erro, o próprio registro de
-- auditoria da falha. Erros de validação anteriores à criação da run (conta
-- inexistente, parâmetros inválidos, permissão negada) continuam sendo
-- levantados normalmente, pois não há nenhuma run para preservar.
...
begin
    ...
    -- grants ao final do arquivo) — ou seja, é um scheduler/serviço de confiança,
    ...
    -- escolhemos uma arbitrariamente).
    ...
    begin
        ...
        loop
            ...
            join public.market_products p
              on p.id = sp.product_id and p.market_account_id = sp.market_account_id
            where sp.market_account_id = p_market_account_id
              and sp.market_store_id = v_store.store_id
            ...
            with elig as (
                select sp.product_id, sp.minimum_stock, sp.is_essential
                from public.market_store_products sp
                join public.market_products p
                  on p.id = sp.product_id and p.market_account_id = sp.market_account_id
                where sp.market_account_id = p_market_account_id
            ...
            leveled as (
                select
                    ...
                    -- usable_stock (saldo utilizavel, nunca negativo; sem historico
                    -- vira 0) em vez do current_stock bruto: um saldo negativo real
                    -- nao deve inflar a sugestao alem do necessario para sair de
                    -- "nada disponivel" ate a meta.
                    greatest(
                        0,
                        greatest(
...
revoke all on function public.market_run_replenishment_batch(uuid, date, text) from public, anon;
grant execute on function public.market_run_replenishment_batch(uuid, date, text) to authenticated;
grant execute on function public.market_run_replenishment_batch(uuid, date, text) to service_role;

comment on function public.market_run_replenishment_batch(uuid, date, text) is
'Executa o batch da Reposicao Inteligente para um Market/reference_date: analisa lojas operacionais e produtos elegiveis, calcula as 5 regras funcionais, persiste o TOP 200 por loja em market_replenishment_candidates e promove o run concluido a is_effective. algorithm_version fixa em v1.';
