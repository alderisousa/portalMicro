...
-- Corrige regressao introduzida em 202609120004/202609120005: candidatos
-- critical/high/medium com suggested_quantity NULL deixaram de ser
-- materializados na Lista de Compras, mesmo sendo necessidade real do
-- batch (NULL = "a definir", nao ausencia de necessidade). Isso fazia uma
-- loja cujos candidatos so tivessem quantidade desconhecida nunca ganhar
-- allocation e, por consequencia, nunca aparecer no seletor de lojas da
-- Lista de Compras.
--
-- Regra restaurada (identica a 202609100016/202609110001/202609120001,
-- que ja tratavam isso corretamente antes da regressao):
-- - o filtro de materializacao e por priority_level (critical/high/medium),
--   nunca por suggested_quantity;
-- - se qualquer loja participante de um produto tiver necessidade
--   desconhecida, o total consolidado do produto tambem fica desconhecido
--   (NULL nunca vira zero por omissao ou por SUM/LEAST/subtracao);
-- - a allocation da loja com necessidade desconhecida e criada com
--   suggested_quantity/warehouse_allocated_quantity/purchase_needed_quantity
--   todos NULL, ficando pendente de revisao manual.
--
-- Nao altera o batch, prioridades, SALES_ACCELERATION ou a regra de ordem
-- ativa/stale (202609130004) -- apenas a materializacao interna.
...
begin
    ...
    -- Necessidade critical/high/medium existe mesmo com quantidade sugerida
    -- ainda desconhecida (NULL): o filtro e por prioridade, nao por
    -- suggested_quantity.
    ...
    with product_totals as (
        select
            c.product_id,
            -- Se qualquer loja participante do produto tiver necessidade
            -- desconhecida, o total consolidado tambem fica desconhecido.
            -- Nunca vira zero por omissao do candidato.
            case
                when count(*) filter (where c.suggested_quantity is null) > 0
                    then null
    ...
    split as (
        select
            oc.*,
            -- suggested_quantity desconhecida nunca produz um rateio de
            -- Galpao calculado: o rateio fica NULL ate revisao manual, em
            -- vez de LEAST/subtracao converterem a incognita em zero.
            case
                when oc.suggested_quantity is null then null
                else least(
                    oc.suggested_quantity,
                    greatest(
                        oc.warehouse_available - oc.previous_known_need,
                        0
                    )
                )::numeric(18,4)
