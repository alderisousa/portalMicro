...
-- Migration: simplifica a regra de estoque minimo (BELOW_MINIMUM, sem
--            depender de is_essential)
-- Data: 2026-09-10
--
-- Contexto:
-- 202609100013_add_replenishment_acceleration_min_units.sql JÁ FOI APLICADA
-- no Supabase de homologação e é imutável a partir de agora.
--
-- Decisão funcional:
-- Qualquer produto com minimum_stock configurado deve gerar necessidade
-- quando o estoque conhecido da loja ficar abaixo desse mínimo — a regra
-- deixa de exigir is_essential. is_essential continua existindo e é usado
-- normalmente por ESSENTIAL_NO_RECENT_SALES (e fica disponível para usos
-- futuros); só deixou de ser um pré-requisito desta regra especifica.
--
-- 1. Estado original de market_store_products.minimum_stock (inspecionado em
--    202608310001_create_market_multitenant_core.sql, linha 100 — migration
--    histórica, já aplicada, NÃO editada por esta migration):
--      minimum_stock numeric(14,3) not null default 0 check (minimum_stock >= 0)
--    NOT NULL com DEFAULT 0 impede distinguir "mínimo não informado" de
--    "mínimo explicitamente zero": qualquer row criada sem tocar essa coluna
--    (ex.: uma row criada só para configurar sale_price ou is_essential) já
--    nasce com minimum_stock = 0, indistinguível de um zero intencional.
--    Correção: minimum_stock passa a aceitar NULL e perde o default 0 — daqui
--    em diante, uma row nova sem minimum_stock explícito fica NULL (não
--    configurado), não 0. O check (>= 0) já tolerava NULL (CHECK só falha em
--    FALSE, nunca em NULL) e não precisou mudar. Nenhum dado histórico é
--    alterado retroativamente: rows já existentes com minimum_stock = 0
--    continuam com 0 (não há como distinguir, a esta altura, quais eram
--    "explícitas" e quais vieram do default antigo — fora do escopo desta
--    correção).
--
-- 2. market_replenishment_candidates (202609100003) também já foi aplicada e
--    é imutável. Seu check
--    market_replenishment_candidates_trigger_reasons_values_check só permitia
--    o literal 'ESSENTIAL_BELOW_MINIMUM' em trigger_reasons; inserir
--    'BELOW_MINIMUM' violaria esse check sem esta correção. A constraint é
--    substituída (drop + add) por uma versão idêntica, trocando apenas
--    'ESSENTIAL_BELOW_MINIMUM' por 'BELOW_MINIMUM' no conjunto permitido.
--
-- 3. Nova regra (renomeada de ESSENTIAL_BELOW_MINIMUM para BELOW_MINIMUM):
--      BELOW_MINIMUM dispara quando minimum_stock is not null
--                          and current_stock is not null
--                          and current_stock < minimum_stock
--    is_essential não interfere: nem exige, nem impede. Sem minimum_stock
--    configurado (NULL), a regra não se aplica. Estoque desconhecido
--    (current_stock NULL) continua nunca tratado como zero — a regra
--    simplesmente não dispara nesse caso.
--
-- Nenhuma outra regra, peso, agravante, classificação, TOP 200, Galpão,
-- suggested_quantity, seleção de fonte, mapping Accesys, segurança, lock ou
-- is_effective foi alterada. STOCKOUT, LOW_COVERAGE, SALES_ACCELERATION e
-- ESSENTIAL_NO_RECENT_SALES permanecem byte-idênticas à versão aplicada em
-- 202609100013.

begin;

-- (1) minimum_stock passa a distinguir "não configurado" (NULL) de
-- "explicitamente zero" (0).
alter table public.market_store_products
    alter column minimum_stock drop not null,
    alter column minimum_stock drop default;

comment on column public.market_store_products.minimum_stock is
'Estoque mínimo configurado para o produto na loja. NULL = não configurado (nenhuma regra de mínimo se aplica); 0 = mínimo explicitamente zero. Usado pela Reposição Inteligente (regra BELOW_MINIMUM).';

-- (2) troca ESSENTIAL_BELOW_MINIMUM por BELOW_MINIMUM no conjunto permitido
-- de trigger_reasons.
alter table public.market_replenishment_candidates
    drop constraint market_replenishment_candidates_trigger_reasons_values_check;

alter table public.market_replenishment_candidates
    add constraint market_replenishment_candidates_trigger_reasons_values_check
    check (
...
-- (3) RPC: BELOW_MINIMUM substitui ESSENTIAL_BELOW_MINIMUM, sem depender de
-- is_essential.
...
begin
    ...
    begin
        ...
        if v_use_synced then
            -- product_id efetivo = coalesce(market_sale_items.product_id,
            -- market_product_mappings.product_id). O LEFT JOIN usa exatamente
            -- as colunas do indice unico parcial
            -- ux_market_product_mappings_integration_product
            -- (market_account_id, integration_id, external_product_id):
            -- no maximo 1 linha de mapping por item de venda, nunca multiplica
            -- sum(i.quantity). Quando i.external_product_id e NULL, a condicao
            -- nunca casa (0 linhas, nao varias) e o item permanece descartavel
            -- pelo filtro de product_id resolvido ausente.
            create temporary table tmp_replenishment_sales on commit drop as
            select
                s.market_store_id as store_id,
                coalesce(i.product_id, mpm.product_id) as product_id,
            ...
            left join public.market_product_mappings mpm
              on mpm.market_account_id = i.market_account_id
             and mpm.integration_id = v_integration_id
             and mpm.external_product_id = i.external_product_id
             and mpm.source_system = 'accesys'
            where s.market_account_id = p_market_account_id
              ...
              and coalesce(i.product_id, mpm.product_id) is not null
              and s.sold_at::date between p_reference_date - (v_sales_lookback_days - 1) and p_reference_date
            group by s.market_store_id, coalesce(i.product_id, mpm.product_id), s.sold_at::date;
        else
            create temporary table tmp_replenishment_sales on commit drop as
            select
        ...
        loop
            ...
            -- explicita e rastreavel (nao ha ausencia silenciosa). rs.* ja
            -- inclui acceleration_min_units_m7 automaticamente.
            ...
            sales_agg as (
                select
                    ...
                    -- sempre o check constraint da tabela de candidatos. sum_m7
                    -- tambem e a quantidade bruta usada pelo piso minimo de
                    -- SALES_ACCELERATION (acceleration_min_units_m7), sem
                    -- derivar novamente de avg_daily_m7 * 7.
            ...
            computed as (
                select
                    ...
                    -- Quantidade bruta vendida em M7 (nao dividida por 7): usada
                    -- pelo piso minimo de SALES_ACCELERATION. avg_daily_m7 abaixo
                    -- continua sendo a media diaria, para as demais formulas.
                    coalesce(sa.sum_m7, 0) as sales_volume_m7,
            ...
            flagged as (
                select
                    ...
                    (
                        -- BELOW_MINIMUM (antes ESSENTIAL_BELOW_MINIMUM): deixou de
                        -- exigir is_essential. Dispara para qualquer produto com
                        -- minimum_stock configurado (not null) cujo saldo conhecido
                        -- fique abaixo dele. Sem minimum_stock configurado, a regra
                        -- nao se aplica; estoque desconhecido nunca e tratado como
                        -- zero (current_stock is not null continua exigido).
                        s.minimum_stock is not null
                    ...
                    (
                        -- SALES_ACCELERATION exige as 3 condicoes simultaneamente:
                        -- 1. piso minimo de volume bruto em M7 (evita ruido de
                        --    percentual alto sobre base muito pequena; ex.: 1 para 2
                        --    unidades ja e +100%). Usa a soma bruta ja calculada em
                        --    sales_agg (sales_volume_m7), sem derivar novamente de
                        --    avg_daily_m7 * 7.
                        -- 2. aceleracao acima do limiar efetivo.
                        -- 3. corroboracao na janela intermediaria M14 (tambem acima
                        --    da referencia de longo prazo M30), aproveitando as tres
                        --    janelas em vez de decidir com uma unica media/pico
                        --    isolado dentro do M7.
                        s.sales_volume_m7 >= v_settings.acceleration_min_units_m7
                        and s.avg_daily_m30 > 0
                    ...
                    (
                        -- is_essential permanece exigido aqui (nao afetado por esta
                        -- migration) e continua disponivel para usos futuros.
                        s.is_essential
                        and (
            ...
            reasoned as (
                select
                    ...
                    array_remove(array[
                        ...
                        case when f.f_below_minimum then 'BELOW_MINIMUM' end,
            ...
            triggered as (
                select
                    ...
                    (
                        ...
                        -- Peso de BELOW_MINIMUM inalterado (+35), so a condicao de
                        -- disparo mudou.
                        (case when r.f_stockout then 50 else 0 end)
                        + (case when r.f_below_minimum then 35 else 0 end)
...
'Executa o batch da Reposicao Inteligente para um Market/reference_date: analisa lojas operacionais e produtos conhecidos/vendidos por elas (via tmp_replenishment_sales, com product_id resolvido por coalesce(item, mapping) na fonte Accesys; market_store_products e configuracao opcional), calcula as 5 regras funcionais (BELOW_MINIMUM nao depende mais de is_essential; SALES_ACCELERATION exige piso minimo de volume M7 via acceleration_min_units_m7), persiste o TOP 200 por loja em market_replenishment_candidates e promove o run concluido a is_effective. suggested_quantity fica NULL quando current_stock e desconhecido. algorithm_version fixa em v1.';
