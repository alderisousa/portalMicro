...
-- Migration: resolve product_id das vendas Accesys via market_product_mappings
-- Data: 2026-09-10
--
-- Contexto:
-- 202609100011_fix_market_replenishment_batch_integration_uuid.sql JÁ FOI
-- APLICADA no Supabase de homologação e é imutável a partir de agora.
--
-- Constatação real da homologação:
-- - market_sale_items.product_id está NULL em 100% dos itens recentes da
--   integração Accesys (4.589/4.589 na amostra observada).
-- - Isso fazia "and i.product_id is not null" descartar TODAS as linhas de
--   venda sincronizadas: tmp_replenishment_sales ficava vazia, e portanto o
--   universo de produtos por loja (introduzido em 202609100010) também
--   ficava vazio — products_analyzed = 0 mesmo com vendas reais.
-- - Não é um problema de cobertura: os 753 produtos externos distintos
--   vendidos têm mapping em market_product_mappings (5.393 mappings, todos
--   com product_id preenchido). O produto simplesmente não foi gravado em
--   market_sale_items.product_id no momento da sincronização, mas o
--   mapeamento existe e é resolvível.
--
-- Correção: ao montar tmp_replenishment_sales pela fonte sincronizada, o
-- product_id efetivo passa a ser
--   coalesce(market_sale_items.product_id, market_product_mappings.product_id)
-- com fallback via LEFT JOIN em market_product_mappings usando exatamente as
-- colunas que o projeto já usa para essa finalidade (mesmas colunas
-- adicionadas e already usadas por market_apply_sales_sync_day/o sync de
-- produtos em 202609030003_add_resumable_market_product_sync.sql):
--   mpm.market_account_id = i.market_account_id
--   mpm.integration_id    = v_integration_id
--   mpm.external_product_id = i.external_product_id  (mesma coluna em ambas
--                                                       as tabelas)
--   mpm.source_system = 'accesys'  (redundante com integration_id, mantido
--                                    como filtro defensivo explícito)
--
-- Por que não duplica linhas de venda:
-- ux_market_product_mappings_integration_product é um índice ÚNICO parcial
-- em (market_account_id, integration_id, external_product_id) — exatamente
-- as três colunas do JOIN (quando ambas integration_id/external_product_id
-- são not null, que é sempre o caso junto, por
-- market_product_mappings_integration_identity_check). Logo, para qualquer
-- linha de market_sale_items, o LEFT JOIN encontra NO MÁXIMO 1 linha de
-- mapping — nunca multiplica o SUM(i.quantity). Quando
-- i.external_product_id é NULL, a condição de junção nunca casa (NULL não
-- é igual a NULL em SQL), então mpm fica inteiramente NULL para essa linha
-- (0 linhas casadas, não múltiplas) e o item continua descartável pelo
-- filtro de product_id resolvido ausente, como já acontecia antes.
--
-- Itens que permanecem sem product_id resolvido (nem item, nem mapping) são
-- descartados exatamente como antes — nenhuma lógica de conciliação nova foi
-- criada e market_sale_items não é alterada retroativamente (somente leitura
-- nesta função).
--
-- Tudo o mais é idêntico à versão aplicada em 202609100011: fonte legada
-- (market_sales_import_rows) inalterada, seleção Accesys vs legado
-- inalterada, universo de produtos vendidos (0010), M7/M14/M30,
-- acceleration_pct, coverage_days, as 5 regras, pesos/agravantes/
-- classificação, TOP 200 e desempate, warehouse_stock e exclusão do Galpão,
-- suggested_quantity nullable, advisory lock, is_effective, contrato de
-- falha e checagem de autorização. Assinatura e grants não mudam.
