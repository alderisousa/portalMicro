...
-- Migration: corrige market_run_replenishment_batch (universo de produtos e
--            suggested_quantity nullable)
-- Data: 2026-09-10
--
-- Contexto:
-- 202609100008_add_market_replenishment_batch_rpc.sql JÁ FOI APLICADA no
-- Supabase de homologação e é imutável a partir de agora — o arquivo local
-- dela foi restaurado ao conteúdo exatamente aplicado. Toda correção da
-- função a partir deste ponto vem em migrations novas via
-- CREATE OR REPLACE FUNCTION, como esta.
--
-- Motivo da correção (constatado na homologação):
-- - market_store_products está vazia em todas as lojas.
-- - market_stock_balance ainda é parcial (não alimentado continuamente).
-- - A fonte hoje confiável é catálogo de produtos + histórico de vendas.
-- Consequência: usar market_store_products como universo obrigatório de
-- produtos por loja (como a versão aplicada em 0008 fazia) faz o batch nunca
-- analisar nenhum produto em nenhuma loja.
--
-- O que muda nesta versão em relação à aplicada em 0008 (só isto — nenhuma
-- outra regra foi alterada):
-- 1. Universo de produtos por loja: passa a ser product_id distinto
--    encontrado em tmp_replenishment_sales para aquela loja (mesma tabela de
--    vendas já pré-computada pela função, já resolvida via
--    market_sale_items.product_id / market_sales_import_rows.product_id —
--    nenhum mapeamento novo é criado), restrito a market_products ativos do
--    Market. Nenhum catálogo paralelo é criado; nenhuma row é inserida
--    automaticamente em market_store_products.
-- 2. market_store_products passa a ser LEFT JOIN opcional (antes era join
--    obrigatório): fornece minimum_stock/is_essential quando existir row
--    ativa; na ausência (ou row inativa), minimum_stock fica NULL (nunca 0 —
--    0 afirmaria "mínimo configurado é zero") e is_essential fica false.
-- 3. suggested_quantity passa a ser NULL sempre que current_stock for NULL
--    (sem histórico de estoque conhecido) — nunca inventa uma sugestão
--    física sem saldo conhecido, mesmo que SALES_ACCELERATION ou
--    ESSENTIAL_NO_RECENT_SALES tenham disparado (nenhuma das duas depende de
--    current_stock). Quando current_stock é conhecido, a fórmula
--    (greatest com saldo utilizável mínimo 0) é a mesma já aplicada em 0008.
--    A coluna já é nullable graças a 202609100009_alter_market_replenishment_
--    candidates_suggested_quantity.sql (mantida como migration separada).
--
-- Tudo o mais é idêntico à versão aplicada em 0008: as 5 regras funcionais,
-- pesos/agravantes/classificação, fórmulas de avg_daily_m7/m14/m30,
-- reference_daily_demand, coverage_days, acceleration_pct, exclusão do
-- Galpão (store_type='warehouse' nunca entra no loop de lojas nem recebe
-- candidate), snapshot de warehouse_stock, ordenação/desempate do TOP 200,
-- lookback dinâmico de vendas (greatest(90, essential_no_sale_days efetivo)),
-- concorrência via advisory lock, promoção de is_effective, contrato de falha
-- (status failed + error_code/error_message sem re-raise) e a checagem de
-- autorização (manual exige owner/admin/manager; scheduled/service_role sem
-- auth.uid() não é bloqueado). Assinatura e grants da função não mudam —
-- CREATE OR REPLACE FUNCTION preserva owner e privilégios já concedidos
-- quando a assinatura não muda, então não há necessidade de repetir
-- REVOKE/GRANT aqui.
